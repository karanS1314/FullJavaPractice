 ...
 //==================================================================================================
     ...
     public static void main(String[] args) {
        ...
        while(t-->0){
            ...
            int mxRem = 0;
            for(int i = 1; i < n - 1; i++){
                int cnt = dp[i - 1] + dp[i] + dp[i + 1];
                int pp = 0;
                long temp = a[i];
                a[i] = a[i - 1];
                pp = isValley(i) + isValley(i - 1) + isValley(i + 1) + isHill(i) + isHill(i - 1) + isHill(i + 1);
                mxRem = Math.max(mxRem , cnt - pp);
            ...
            chonch = Math.max(0 , chonch - mxRem);
            System.out.println(chonch);
        }
     }
     static int isValley(int i){
        int n = a.length;
        if(i >= 1 && i < n - 1 && a[i] < a[i - 1] && a[i] < a[i + 1]){
            return 1;
        }
        return 0;
     }
     static int isHill(int i){
        int n = a.length;
        if(i >= 1 && i < n - 1 && a[i] > a[i - 1] && a[i] > a[i + 1]){
            return 1;
        }

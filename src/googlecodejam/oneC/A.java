     ...
     public static void main(String[] args) {
        ...
        while(t-->0){
            ...
            for(int i=0;i<n-1;i++){
                ...
                if(p[i+1] - p[i]<=2 && p[i+1] - p[i]>1&& mx1 == 0){
                    l = i;
                    mx1 = 1;
                }
            }
            ...
            if(l!=-1){
                for(int i=0;i<n-1;i++){
                    ...
                    if(p[i+1] - p[i]<=2 && p[i+1] - p[i]>1 && mx2 == 0){
                        mx2 = 1;
                    }
                }
                int mx3 = Math.min(mx1 , mx2);
                mx3 = Math.max(p[0]-2, k-p[n-1]);
                res = (double)(mx3 + Math.max(mx1 , mx2))/(double)(k);
            }

package codeForces.codeForcesedu111;
     ...
     public static void main(String[] args) {
        ...
        while(t-->0){
            ...
            for(int i = 0; i < k; i++){
                ...
                mi = Math.min(mi , find(ca , k));
            }

            System.out.println(mi);
        }
     }
     static int find(char ca[] , int k){
         int i = 0;
         char x = ca[0];
         int min = imx;
         while(i < ca.length){
            while(i < ca.length && ca[i] == x && ca[i] > (char)(97 + k)){
                i++;
            }
            if(i < ca.length)
                x = ca[i];
            int j = i;
            while(i < ca.length && ca[i] == x){
                i++;
            }

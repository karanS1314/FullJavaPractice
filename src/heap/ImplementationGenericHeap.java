package heap;
 ...
 public class ImplementationGenericHeap {
     static class Pair implements Comparable<Pair>{
         // comaprable is interface -- interface is a contract of functions that need to be followed by the child class
 ...
 //==================================================================================================
     ...
     public static void main(String[] args) {
        ...
        while(t-->0){
            ...
            MyPriorityQueue<Pair> pq = new MyPriorityQueue(new RnoComp());
            ...
            for(int i = 0; i < n; i++){
                System.out.println(b[i].rNo + " " + b[i].name);
            }
            
        }
     }
     static class RnoComp implements Comparator<Pair>{
     ...
     static class MyPriorityQueue<T>{
        ...
        boolean isSmaller(int i , int j){
            if(comp == null){ // the comparable default in the pair class
        ...
        public void downheapify(int ind){
            int lci = 2 * ind + 1;
            int rci = 2 * ind + 2;

            int min = ind;
            if(lci < al.size() && isSmaller(lci , ind)){
                min = lci;
                swap(lci , ind);
            }

            if(rci < al.size() && isSmaller(rci , ind)){
                min = rci;
                swap(rci , ind);
            }
            if(min != ind){

package codeForces.virtual.cf709div1;
 ...
 //==================================================================================================
     ...
     static boolean solve(ArrayList<Integer> ar[] , int ind , int fre[] , ArrayList<Integer> al){
        if(ind >= m){
            res = new ArrayList<>(al);
            return true;
        }
        
        for(int i = 0; i < ar[ind].size(); i++){
            // chose
            if(fre[ar[ind].get(i)] + 1 <= m / 2){
                fre[ar[ind].get(i)]++;
                al.add(ar[ind].get(i));
                if(solve(ar , ind + 1 , fre , al)){
                    return true;
                }
                al.remove(al.size() - 1);
                fre[ar[ind].get(i)]--;

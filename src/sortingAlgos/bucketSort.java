package sortingAlgos;
 ...
 public class bucketSort {
     public static void main(String[] args) {
        ...
        int bSize = (int)Math.ceil((mx - min)*1.0/(n-1)*1.0)+1;

        ArrayList<Integer> buc[] = new ArrayList[bSize];
        ...
        mx = buc[0].get(buc[0].size()-1);
        for(int i=1;i<bSize;i++){
            if(buc[i].size()>0){
                res = Math.max(buc[i].get(0)-mx, res);
                mx = buc[i].get(buc[i].size()-1);
            }
        }

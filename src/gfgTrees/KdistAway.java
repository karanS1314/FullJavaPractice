package gfgTrees;
 ...
 public class KdistAway {
    ...
    static Node generateTree(int x){
        if(x == -1){
            return null;
        }
        ...
        System.out.println("Enter left to the node" + x);
        int lData = sc.nextInt();
        node.left = generateTree(lData);


        System.out.println("Enter right to the node" + x);
        int rData = sc.nextInt();
        node.right = generateTree(rData);

        return node;
    }

    static boolean ntrp(int x , Node node){
        if(node == null){
            return false;
        }
        ...
        boolean leftSide = ntrp(x , node.left);
        if(leftSide){
            path.add(node);
            return true;
        }

        boolean rightSide = ntrp(x , node.right);
        if(rightSide){
            path.add(node);
            return true;
        }
    ...
    static void printXnodesDown(Node node , int x , Node block){
        ...
        printXnodesDown(node.left, x - 1, block);
        printXnodesDown(node.right, x - 1, block);
    }

    public static void main(String[] args) {
        
        int rootData = sc.nextInt();
        Node root = generateTree(rootData);

        System.out.println("Your tree is ready");

        int disFrom = sc.nextInt();
        int k = sc.nextInt();

        boolean ntrp = ntrp(disFrom , root);

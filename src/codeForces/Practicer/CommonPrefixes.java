 ...
 public class CommonPrefixes {
     ...
     public static void main(String[] args) {
        ...
        while(t-->0){
            ...
            StringBuffer sa[] = new StringBuffer[n+1];
            
            sa[0] = new StringBuffer("");
            for(int i=0;i<100;i++){
                sa[0].append("a");
            }

            for(int i=1;i<n+1;i++){
                sa[i] = new StringBuffer(sa[i-1].toString());
                if(sa[i-1].charAt(a[i-1]) == 'a'){
                    sa[i].replace(a[i-1] , a[i-1] + 1, "b");
                }
                else{
                    sa[i].replace(a[i-1] , a[i-1] + 1, "a");
                }
            }

            for(StringBuffer s : sa){

package codeForces.codeForces726;
 ...
 //==================================================================================================
     static String create(String s , int k){
         StringBuilder temp = new StringBuilder("");
         int n = s.length();
         for(int i = 0; i < k; i++){
             temp.append(s.charAt(i % n));
         }
     ...
     public static void main(String[] args) {
        ...
        while(t-->0){
            ...
            StringBuilder temp = new StringBuilder(create(s , k));
            for(int i = 1; i < n + 1; i++){
                if(firstGreater(temp , create(s.substring(0 , i) , k))){
     ...
     static boolean firstGreater(StringBuilder sb , String s){
        String pre = s;
        for(int j = 0; j < sb.length(); j++){
            if(sb.charAt(j) < pre.charAt(j)){
                return false;
            }
            else if(sb.charAt(j) > pre.charAt(j)){
                return true;
            }
        }
        return false;

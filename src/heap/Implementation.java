 ...
 //==================================================================================================
     ...
     static class MyPriorityQueue{
        ArrayList<Integer> al;
            // treat this array as tree
            // lci = 2*pi + 1;
            // rci = 2*pi + 2;

            // pi = (anychildIndex - 1) / 2;

            // addition to heap - logn
            // remove from heap - logn
            // peek from heap - logn
        ...
        public void downheapify(int ind){
            ...
            int min = ind;
            if(lci < al.size() && al.get(lci) < al.get(ind)){
                min = lci;
                swap(lci , ind);
            }

            if(rci < al.size() && al.get(rci) < al.get(ind)){
                min = rci;
                swap(rci , ind);
            }
            if(min != ind){

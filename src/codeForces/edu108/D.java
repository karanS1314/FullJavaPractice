 ...
 public class D {
    static class Pair implements Comparable<Pair>{
     ...
     public static void main(String[] args) {
        ...
        while(t-->0){
            int n = sc.nextInt();
            int u[] = sc.readArray(n);
            int s[] = sc.readArray(n);
 
            Pair p[] = new Pair[n];
            for(int i=0;i<n;i++){
                p[i] = new Pair(u[i] , s[i]);
            }
            ...
            int tsum = 0;
             ...
             int mx = 1;
             for(int i=0;i<n;i++){
                 if(data[p[i].u].size()>0){
                    int x = data[p[i].u].get(data[p[i].u].size()-1);
                    data[p[i].u].add(p[i].s + x);
                 }
                 else{
                     data[p[i].u].add(p[i].s);
                 }
                 mx = Math.max(mx , data[p[i].u].size());
             }

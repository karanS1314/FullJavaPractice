package codeForces.codeforces735;
 ...
 //==================================================================================================
    ...
    static int Mex(int n , int m){
        int x = 0;
        int res = mod;
        for(int i = 29; i >= 0; i--){
            int a = (n & (1 << i)); // n ki ith bit check
            int b = (m & (1 << i)); // m //

            if(a > 0 && b > 0){ // agar dono bit 1
                continue;
            }
            else if(a > 0){ // agar n ki bit 1 aur m ki 0
                return Math.min(res , x);
            }
            else if(b > 0){ // n ki 0 aur m ki 1
                x += (1 << i);
            }
            else{ // dono zero
                if(x + (1 << i) < res){

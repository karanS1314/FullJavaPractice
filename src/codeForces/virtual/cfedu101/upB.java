package codeForces.virtual.cfedu101;
 ...
 //==================================================================================================
     public static void main(String[] args) {
        ...
        while(t-->0){
            ...
            // rws mei r and cols mei b
            int res = 0;

            for(int i = 1; i < n + 1; i++){  // filling the rows
                dp[i][0] = dp[i - 1][0] + r[i - 1];
                res = Math.max(dp[i][0] , res);
            }
            for(int j = 1; j < m + 1; j++){  // filling the cols
            ...
            for(int i = 1; i < n + 1; i++){
                for(int j = 1; j < m + 1; j++){
                    dp[i][j] = Math.max(dp[i - 1][j] + r[i - 1] , dp[i][j - 1] + b[j - 1]);
     ...
     static int solve(int dp[][] , int n , int r[] , int m , int b[] , int i , int j){
        if(i == n && j == m){
            return 0;
        }
        if(i == n){
            ...
            return dp[i][j] = solve(dp , n , r , m , b , i , j + 1); 
        }
        if(j == m){
            if(dp[i][j] != imi){
                return dp[i][j];
            }
            return dp[i][j] = solve(dp , n , r , m , b , i + 1 , j); 
        }
        ...
        // choosing ith from r
        int up = solve(dp , n , r , m , b , i + 1 , j);
        // choosing jth from b
        int down = solve(dp , n , r , m , b , i , j + 1);
        
        return dp[i][j] = Math.max(Math.max(up + r[i] , down + b[j]) , 0);
     }

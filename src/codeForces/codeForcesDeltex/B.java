package codeForces.codeForcesDeltex;
 ...
 public class B {
     ...
     public static void main(String[] args) {
        ...
        while(t-->0){
            ...
            for(int i = n; i > 0; i--){
                for(int j = i + 1; j <= n; j++){
                    while(true && a[i] > 0 && Math.abs(a[j] - b[j]) / a[i] <= 5000){
                        if(a[j] - a[i] < b[j]){
                            break;
                        }
                        res.add("2 " + i + " " + j);
            ...
            for(int i = 1; i <= n; i++){
                for(int j = i + 1; j <= n; j++){
                    if(i == 1){
                        System.out.println("1 :" + (Math.abs(a[i] - b[i]) / (a[j])));
                    }
                    while(a[j] + a[i] >= b[i] && a[j] < 0 && (Math.abs(a[i] - b[i]) / (a[j])) <= 5000){
                        res.add("1 " + i + " " + j);
                        a[i] += a[j];
                    }
                }

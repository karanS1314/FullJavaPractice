package codeHacks;
 ...
 public class PrintPathsKnapsack {
     static class Pair implements Comparable<Pair>{
         int a;
         int b;
         String psf;
 
         Pair(int a , int b , String psf){
             this.a = a;
             this.b = b;
             this.psf = psf;
         }
        ...
        while(t-->0){
            ...
            while(!q.isEmpty()){
                ...
                if(i == 0 || j == 0){
                    ...
                    int exc = dp[i - 1][j];
                    int inc = dp[i - 1][j - w[i - 1]] + v[i - 1];

                    if(dp[i][j] == inc){ // agar include vale hua hai mera weight i , j vala iss dp[i][j] ke lie
                        q.add(new Pair(i - 1 , j - w[i - 1] , " " + (i - 1) + s));
                    }
                    if(dp[i][j] == exc){ // nhi hua include
                        q.add(new Pair(i - 1 , j , s));
                    }
                }
            }
        }
